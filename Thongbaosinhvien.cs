...
namespace QL_GiangDay
{
    public partial class Thongbaosinhvien : UserControl
    {
        ...
        {
            DataTable dt = qlgd.DocBang("SELECT tb.ThongBaoID, tb.TieuDe, tb.NoiDung, tb.NgayGui, tb.TrangThai, tbsv.SinhVienID AS NguoiNhanID, sv.HoTen AS NguoiNhan, tbsv.LoaiNguoiNhan\r\nFROM ThongBao tb\r\nJOIN ThongBao_SinhVien tbsv ON tb.ThongBaoID = tbsv.ThongBaoID\r\nJOIN SinhVien sv ON tbsv.SinhVienID = sv.SinhVienID");
            try
            {
                if (dt != null)
                {
                    dataGridView_TBSV.DataSource = dt;
                }
            }
            catch (Exception ex)
            {
                ...
                if (MessageBox.Show("Bạn có chắc chắn muốn thêm thông tin?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        SqlParameter[] parameters = new SqlParameter[] {
                            ...
                            new SqlParameter("@SinhVienID", SqlDbType.NVarChar) {Value = txtMaNgNhan.Text},
                            new SqlParameter("@LoaiNguoiNhan", SqlDbType.NVarChar) {Value = txtLoaiDoituong.Text},
                        };

                        string query = "EXEC ThemThongBao_SV @ThongBaoID, @TieuDe, @NoiDung, @NgayGui, @TrangThai, @SinhVienID, @LoaiNguoiNhan";
        ...
        private void Hienthi()
        {
            ...
            loadComboBox();
            Cbb_TrangThai.SelectedIndex = -1;
            dataGridView_TBSV.ReadOnly = true;
        }
        private void clickdata()
        {
            txtMaTB.Text = dataGridView_TBSV.CurrentRow.Cells[0].Value.ToString();
            txtTieuDe.Text = dataGridView_TBSV.CurrentRow.Cells[1].Value.ToString();
            txtNoidung.Text = dataGridView_TBSV.CurrentRow.Cells[2].Value.ToString();
            txtNgayGui.Text = dataGridView_TBSV.CurrentRow.Cells[3].Value.ToString();
            Cbb_TrangThai.SelectedValue = dataGridView_TBSV.CurrentRow.Cells[4].Value.ToString();
            txtMaNgNhan.Text = dataGridView_TBSV.CurrentRow.Cells[5].Value.ToString();
            txtHotenNN.Text = dataGridView_TBSV.CurrentRow.Cells[6].Value.ToString();
            txtLoaiDoituong.Text = dataGridView_TBSV.CurrentRow.Cells[7].Value.ToString();
        }
        private void Capnhat()
        {
            ...
            else if (MessageBox.Show("Bạn có chắc chắn muốn cập nhật thông tin?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    string query = "EXEC CapNhatThongBao_SV @ThongBaoID, @TieuDe, @NoiDung, @NgayGui, @TrangThai, @SinhVienID, @LoaiNguoiNhan";
        ...
        private void Xoa()
        {
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa thông tin?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    string query = "EXEC XoaThongBao_SV @ThongBaoID";
        ...
        private void XuatExcel()
        {
            ...
            for (int i = 1; i < dataGridView_TBSV.Columns.Count + 1; i++)
            {
                worksheet.Cells[1, i] = dataGridView_TBSV.Columns[i - 1].HeaderText;
            }

            // Ghi dữ liệu từ DataGridView vào Excel
            for (int i = 0; i < dataGridView_TBSV.Rows.Count; i++)
            {
                for (int j = 0; j < dataGridView_TBSV.Columns.Count; j++)
                {
                    worksheet.Cells[i + 2, j + 1] = dataGridView_TBSV.Rows[i].Cells[j]?.Value?.ToString() ?? "";
                }
            }
        ...
        private void dataGridView_TBSV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            clickdata();
        }
        ...
        private void txtMaNgNhan_TextChanged(object sender, EventArgs e)
        {
            ...
            DataTable db = qlgd.DocBang("select hoten from sinhvien where sinhvienid like N'" + nguoinhanid + "'");
            if (db.Rows.Count < 1)
            {
                txtHotenNN.Text = "";
                return;
        ...
        private void txtHotenNN_TextChanged(object sender, EventArgs e)
        {
            ...
            DataTable db = qlgd.DocBang("select sinhvienid from sinhvien where hoten like N'" + nguoinhan + "'");
            if (db.Rows.Count < 1)
            {
                txtMaNgNhan.Text = "";
                return;
            ...
            {
                txtMaNgNhan.Text = db.Rows[0]["sinhvienid"].ToString();
            }
        }
    }
}
